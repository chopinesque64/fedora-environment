...
#+begin_src emacs-lisp
  (general-define-key
   ...
   "D" '(npg/mark-done-and-archive :which-key "done and archive")
...
Here's a shortcut to mark a task done and put it the archive. A leader-suffix
=D= is defined for both the agenda and Org documents.
#+BEGIN_SRC emacs-lisp
  (defun npg/mark-done-and-archive ()
    "Mark the state of a task item as DONE and archive it."
    (interactive)
    (org-todo 'done)
    (org-archive-subtree))
#+END_SRC
...
** Archive
Note that if we archive, clocking data won't be around.
** Clock
