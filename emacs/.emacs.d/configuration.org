...
#+end_src
 ...
 Derive the agenda from =index.org= (the project index) the calendar file, and
 the journal.
#+begin_src emacs-lisp
  (setq org-agenda-files (list org-index-file org-caldav-inbox org-jot-file))
#+end_src
...
#+begin_src emacs-lisp
  (setq org-capture-templates
    ...
    ("c" "clock minitask" entry (clock)
     "* NEXT %?\n%U\n" :clock-in t :clock-keep t)
    ("n" "Next" entry (file+headline org-index-file "INBOX")
      "* NEXT %?\nDEADLINE: %t\n%U\n%a\n" :clock-in t :clock-resume t :immediate-finish t)
    ("m" "Meeting" entry (file+datetree org-jot-file)
     "* MEETING: %? :MEETING:\n%U\n" :clock-in t :clock-resume t)
